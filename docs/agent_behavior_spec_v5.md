...
#### R1.web: Dashboard tylko do odczytu

Web może agregować i prezentować dane CRM z Google Sheets / Calendar, ale nie
może ich mutować. Dotyczy to również `/dashboard/decyzje-preview`: widok
pokazuje klientów wymagających decyzji i kieruje handlowca do Telegrama.

Zmiana statusu, dopisanie notatki, oznaczenie kontaktu i utworzenie spotkania
odbywają się wyłącznie w Telegramie, przez standardową kartę potwierdzenia R1.
FastAPI nie wystawia endpointów POST dla panelu Decyzje. Web może mutować tylko
dane systemowe i konta (np. profil, billing, onboarding, szablony ofert), nigdy
CRM source-of-truth.
...
> **Active post-MVP slice (live od 25.04.2026):** voice transcription jako input adapter — OpenAI STT (`gpt-4o-transcribe` default, `whisper-1` fallback) + post-pass polskich nazwisk (Claude haiku) + 2-button confirm card (Zapisz/Anuluj, bez procentu pewności). Po potwierdzeniu transkrypcja idzie przez normalny text path (`handle_text(text_override=...)`). Voice nie jest odrębnym intent type — podlega standardowej intent classification. Voice-specific richer flows (proactive voice responses, voice-only commands) zostają vision/POST-MVP.
...
Dla `Drive photos` — photo/image-document jest aktywnym post-MVP slice. Zdjęcie z podpisem `Jan Kowalski Warszawa` może od razu wskazać klienta, ale pierwsze zapisanie do Drive zawsze przechodzi przez kartę `✅ Zapisać`. Po potwierdzeniu agent informuje, że przez 15 minut kolejne zdjęcia bez opisu trafią do tego klienta; zmiana klienta wymaga podpisu `zdjęcia do [imię nazwisko miasto]` albo jednoznacznego imię+nazwisko+miasto. Dla `multi-meeting` — w MVP ścieżka nie jest aktywna w runtime; router odrzuca (`IntentType.MULTI_MEETING`) z prośbą o jedno spotkanie naraz. Voice transcription — LIVE od 25.04.2026 jako input adapter (handle_voice → OpenAI STT → post-pass → confirm card → handle_text text_override).
