...
Voice transcription is **live as an input adapter** since 25.04.2026 (OpenAI STT with `gpt-4o-transcribe` default and `whisper-1` fallback → Polish name post-pass via Claude haiku → 2-button confirm card without confidence percentage; confirmed transcription flows through normal text path via `handle_text(text_override=...)`).
