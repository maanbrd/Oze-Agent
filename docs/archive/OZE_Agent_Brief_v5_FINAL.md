> ⚠️ **ARCHIWUM — NIE JEST ŹRÓDŁEM PRAWDY**
>
> Ten dokument został zamrożony 10.04.2026 wieczorem. Po 4 rundach testów manualnych część decyzji tutaj została zmieniona — w szczególności:
>
> - **Specs techniczne** (metraż domu/dachu, kierunek dachu, zużycie kWh) → teraz idą do `Notatki`, a nie do dedykowanych kolumn.
> - **Moc produktu** (6kW, 8kW, 10kWh) → trafia do `Notatki` razem z resztą specs ("moc PV 8kW"), a nie osobna kolumna `moc_kw`, ani nie doklejona do nazwy produktu. Kolumna `Produkt` zawiera tylko typ produktu (`PV`, `Pompa ciepła`, `Magazyn energii`, `PV + Magazyn`).
> - **R4** ("po add_client zawsze pytaj o następny kontakt") → **usunięta**. R4 w nowej wersji oznacza **identyfikacja klienta po imieniu+nazwisku+mieście**.
> - **Schema Google Sheets** — patrz `docs/poznaj_swojego_agenta_v5_FINAL.md` sekcja "Arkusz klientów".
>
> **Aktualne źródło prawdy (w kolejności priorytetu):**
> 1. `docs/SOURCE_OF_TRUTH.md` — mapa dokumentacji i decision log
> 2. `docs/agent_behavior_spec_v5.md` — zachowanie agenta + 52 testy akceptacyjne
> 3. `docs/agent_system_prompt.md` — ton + wzorce odpowiedzi
> 4. `docs/CURRENT_STATUS.md` — aktualny stan implementacji
>
> Ten plik zostawiamy jako historyczny artefakt (kontekst dlaczego podejmowano pewne decyzje), **nie jako wytyczne**. Jeśli czytasz to szukając odpowiedzi na pytanie "jak agent ma się zachować", idź do SOURCE_OF_TRUTH.md.
