...
@pytest.mark.asyncio
async def test_linking_code_reports_already_used_when_update_hits_unique_conflict(monkeypatch):
    replies: list[str] = []
    update = MagicMock()
    user = _linked_user()
    lookups = iter([None, {"id": "other-user"}])
    ...
    monkeypatch.setattr(
        start,
        "get_user_by_telegram_id",
        lambda _telegram_id: next(lookups),
    )
    monkeypatch.setattr(start, "update_user", lambda _user_id, _payload: None)
